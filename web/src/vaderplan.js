...
export function vaderPlan(rd, { costs = VADER_COSTS, dailyGearEnergy = 480, lv } = {}) {
  ...
  const units = (LV.units || []).map(u => {
    ...
    const curRelic = live ? live.rl : (u.relic || 0);   // fallback embebido si no está en RD (como vaderProgress)
    const curGear = live ? live.g : (u.gear || 0);
    const tgtRelic = u.need, tgtGear = 13;
  ...
  return { units, order, totals: { relicGap, gearGap, days, weeks: Math.ceil(days / 7), unlocked: byName.has("Lord Vader"), dailyGearEnergy: energy } };
}
